...
function main() {
  const data = getData();
  const m = data[0][0]; //바구니의 갯수
  const n = data[0][1]; //반복횟수
  let arr = [];
  //1~m까지 배열로 만듬
  for (let i = 0; i < m; i++) {
    arr[i] = i + 1;
  ...
  for (let i = 0; i < n; i++) {
    const rowArr = data[i + 1];
    const a = rowArr[0] - 1; // 뒤집기 시작할 인덱스
    const b = rowArr[1] - 1; // 여기 까지 뒤집음

    const reversed = arr.slice(a, b + 1).reverse(); //slice메소드는 a부터b-1까지의 메소드를 반환하기 때문에 b인덱스까지 포함하려면 b+1을 해야한다.
    arr = [...arr.slice(0, a), ...reversed, ...arr.slice(b + 1)]; //자바스크립트의 slice메소드는 시작 인덱스와 끝 인덱스가 같으면 빈 배열이 반환된다.
  }
  console.log(...arr);
}
main();
